...
vars(F, [F]) :- atom(F), !.
vars(F ekv G, Vars) :- vars(F, Vars0), vars(G, Vars1), merge(Vars0, Vars1, Vars).
vars(F imp G, Vars) :- vars(F, Vars0), vars(G, Vars1), merge(Vars0, Vars1, Vars).
vars(F or G, Vars) :- vars(F, Vars0), vars(G, Vars1), merge(Vars0, Vars1, Vars).
vars(F and G, Vars) :- vars(F, Vars0), vars(G, Vars1), merge(Vars0, Vars1, Vars).
vars(  non G, Vars) :- vars(G, Vars).

genModel(x1, x2) :- genModel(x1, x2).
eval(x, y, c) :- eval(x,y,c).